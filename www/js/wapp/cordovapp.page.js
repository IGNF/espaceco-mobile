/*
   _________                  .___            _____                 
   \_   ___ \  ___________  __| _/_______  __/  _  \ ______ ______  
   /    \  \/ /  _ \_  __ \/ __ |/  _ \  \/ /  /_\  \\____ \\____ \ 
   \     \___(  <_> )  | \/ /_/ (  <_> )   /    |    \  |_> >  |_> >
    \______  /\____/|__|  \____ |\____/ \_/\____|__  /   __/|   __/ 
           \/                  \/                  \/|__|   |__|    
 */
/**
 * @classdesc
...
/* Version for older browsers * /
	...
	var dw = $(document).width();
	var dh = $(document).height();
	var e = Math.min ((dw-20)/w , (dh-20)/h);

	var nw = img.get(0).naturalWidth; 
	var nh =  img.get(0).naturalHeight; 
	ne = Math.min ((dw-20)/nw , (dh-20)/nh);
	
	var bimg = $("<img>").attr("src",img.attr("src"))
						.css({ position:"relative", top:offset.top, left:offset.left, width:w, height:h })
	fs.append(bimg);
	setTimeout(function()
	{	fs.css("opacity",1);
		bimg.width(w*e)
			.height(h*e)
			.css({ border:0, left:(dw-w*e)/2 , top:(dh-h*e)/2 });
	},200);
/**/
}
